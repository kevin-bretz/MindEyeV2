...
# Low-level submodule (optional - only if model was trained with blurry_recon)
try:
    all_blurryrecons = torch.load(f"evals/{model_name}/{model_name}_all_blurryrecons.pt")
    has_blurry_recons = True
    print("Loaded blurry reconstructions")
except FileNotFoundError:
    all_blurryrecons = None
    has_blurry_recons = False
    print("No blurry recons found (model trained with --no-blurry_recon)")
...
if all_recons.shape[-1] != imsize:
    all_recons = transforms.Resize((imsize,imsize))(all_recons).float()
if has_blurry_recons and all_blurryrecons.shape[-1] != imsize:
    all_blurryrecons = transforms.Resize((imsize,imsize))(all_blurryrecons).float()

if "enhanced" in model_name_plus_suffix and has_blurry_recons:
...
# Try to load optional intermediate features
try:
    all_backbones = torch.load(f"evals/{model_name}/{model_name}_all_backbones.pt")
    has_backbones = True
    print("Loaded all_backbones.pt")
except FileNotFoundError:
    all_backbones = None
    has_backbones = False
    print("all_backbones.pt not found, skipping backbone visualizations")

try:
    all_prior_out = torch.load(f"evals/{model_name}/{model_name}_all_prior_out.pt")
    has_prior_out = True
    print("Loaded all_prior_out.pt")
except FileNotFoundError:
    all_prior_out = None
    has_prior_out = False
    print("all_prior_out.pt not found, skipping prior visualizations")
...
if has_prior_out:
    ...
    print("Skipping Prior_Out UMAP visualization (file not found)")
...
if has_backbones:
    ...
    print("Skipping Backbones UMAP visualization (file not found)")
