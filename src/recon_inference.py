...
model.backbone = BrainNetwork(h=hidden_dim, in_dim=hidden_dim, seq_len=1, 
                          ...
                          blurry_recon=blurry_recon, n_blocks=n_blocks) 
...
converter_path = None
if os.path.exists(f"{cache_dir}/bigG_to_L_epoch8.pth"):
    converter_path = f"{cache_dir}/bigG_to_L_epoch8.pth"
elif os.path.exists("bigG_to_L_epoch8.pth"):
    converter_path = "bigG_to_L_epoch8.pth"
else:
    raise FileNotFoundError("Cannot find bigG_to_L_epoch8.pth in cache_dir or current directory")
...
# Use fp32 for inference to avoid NaN from diffusion prior with mixed precision training
with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.float32):
    for batch in tqdm(range(0,len(np.unique(test_images_idx)),minibatch_size)):
        ...
        # Save diffusion prior outputs
        if all_prior_out is None:
            all_prior_out = prior_out.cpu()
        else:
            all_prior_out = torch.vstack((all_prior_out, prior_out.cpu()))
        
        # Generate captions in fp32 for numerical stability
        with torch.cuda.amp.autocast(enabled=False):
            pred_caption_emb = clip_convert(prior_out.float())
            # Debug: check if embeddings are valid
            if batch == 0:
                print(f"prior_out shape: {prior_out.shape}, range: [{prior_out.min():.3f}, {prior_out.max():.3f}]")
                print(f"pred_caption_emb shape: {pred_caption_emb.shape}, range: [{pred_caption_emb.min():.3f}, {pred_caption_emb.max():.3f}]")
...
torch.save(all_backbones,f"evals/{model_name}/{model_name}_all_backbones.pt")
