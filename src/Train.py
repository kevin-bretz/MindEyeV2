...
if utils.is_interactive():
    ...
    jupyter_args = f"--data_path=/weka/proj-medarc/shared/mindeyev2_dataset \
                    ...
                    --n_blocks=4 --max_lr=3e-4 --max_grad_norm=1.0 --mixup_pct=.33 --num_epochs=150 --no-use_image_aug \
...
for epoch in progress_bar:
    ...
    for train_i in range(num_iterations_per_epoch):
        with torch.cuda.amp.autocast(dtype=data_type):
            ...
            if torch.isnan(loss):
                print(f"NaN loss encountered at step {i} in epoch {epoch}")
                if use_prior: print(f"loss_prior: {loss_prior.item() if isinstance(loss_prior, torch.Tensor) else loss_prior}")
                if clip_scale > 0: print(f"loss_clip: {loss_clip.item() if isinstance(loss_clip, torch.Tensor) else loss_clip}")
                if blurry_recon: 
                    print(f"loss_blurry: {loss_blurry.item() if isinstance(loss_blurry, torch.Tensor) else loss_blurry}")
                    print(f"loss_blurry_cont: {cont_loss.item() if isinstance(cont_loss, torch.Tensor) else cont_loss}")
    ...
    if local_rank==0:
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=data_type): 
            for test_i, (behav, past_behav, future_behav, old_behav) in enumerate(test_dl):  
                ...
                if torch.isnan(loss):
                    print(f"NaN loss encountered at validation step {test_i} in epoch {epoch}")
                    if use_prior: print(f"loss_prior: {loss_prior.item() if isinstance(loss_prior, torch.Tensor) else loss_prior}")
                    if clip_scale > 0: print(f"loss_clip: {loss_clip.item() if isinstance(loss_clip, torch.Tensor) else loss_clip}")
