...
Outputs:
  evals/{model_name}/{model_name}_all_brain_opt_recons_latent.pt
"""
...
# NEW: no --brain_region argument — we use full voxels + ridge instead of ROI mask
...
# NEW: no brain region mask needed — ridge handles dimensionality reduction
...
# ---------------------------------------------------------------------------
# NEW: Load MindEye2 ridge regression weights from checkpoint
# ---------------------------------------------------------------------------
...
print(f"Ridge weight shape: {ridge_weight.shape}")  # e.g. (4096, 14278)

# Free the checkpoint from memory immediately — it's large
del ckpt, state
...
# NEW: replaced score_candidates entirely
def score_candidates(candidates, measured_voxels):
    """
    Score candidates in MindEye2 latent space instead of raw voxel space.
    ...
    Steps:
      ...
      2. Both predicted and real voxels are projected through MindEye2 ridge
         into the shared latent space (hidden_dim)
      3. Cosine similarity in latent space is the score

    Returns:
        scores     : (N,) cosine similarity — higher = better latent match
    ...
    # Project real voxels into latent space once — same for all candidates
    with torch.no_grad():
        z_real = apply_ridge(measured_voxels.to(device))            # (hidden_dim,)
        z_real = z_real / z_real.norm().clamp(min=1e-8)             # L2 normalise

        # Project all candidate predicted voxels at once (batch)
...
# ---------------------------------------------------------------------------
TIMEPOINT_SCHEDULE = [20, 15, 12, 9, 6][:n_iterations]


# ---------------------------------------------------------------------------
# Main loop — only change is measured_beta_masked → full measured_voxels
# and checkpoint saving added
# ---------------------------------------------------------------------------
resume_path = f"evals/{model_name}/{model_name}_all_brain_opt_recons_latent.pt"  # NEW name
...
for img_idx in tqdm(range(start_idx, len(uniq_imgs)), desc="BOI latent"):
    ...
    measured_voxels = test_voxels_averaged[img_idx]      # NEW: full voxels, no masking
    ...
    for timepoint in TIMEPOINT_SCHEDULE:
        ...
        scores, pred_stds, _ = score_candidates(candidates, measured_voxels)  # NEW
    ...
    # NEW: checkpoint every 50 images so a cancelled job can resume
    if (img_idx + 1) % 50 == 0:
